...
@Component({
  selector: 'app-persnal-info-two',
  templateUrl: './persnal-info-two.page.html',
  styleUrls: ['./persnal-info-two.page.scss'],
})
export class PersnalInfoTwoPage implements OnInit {
  //Create FormGroup
  ...
  constructor(
    ...
    localStorage.setItem('hasSeenPersonalInfo', 'true');
          ...
          (res: any) => {
            if (res.hasOwnProperty('operation')) {
              if (res.operation === 'success') {
                ...
                console.log("calling setMapMarker manually from persanl info two page remove");
                this.messageService.presentSuccessToast(
                  'Your information updated successfully'
                );
                this.route.navigate(['/tabs/profile', { markerStatus: '1' }]);
              }
            }
            this.loaderService.hideLoader();
          },
